...
def average_checkpoints(
    ...
    if len(checkpoint_list) == 1:
        # Load and return the state dict directly if only one checkpoint is provided
        return parameter_loader(checkpoint_list[0].paramfiles[recoverable_name], map_location=device)
...
def load_first_checkpoint(checkpoint_list, recoverable_name, parameter_loader=torch.load):
    """
    Load the state dictionary directly from the first checkpoint for a specified recoverable.

    Arguments:
    checkpoint_list : list
        List of checkpoints to consider, sorted in the order of preference.
    recoverable_name : str
        The name of the recoverable, the parameters of which are to be loaded.
    parameter_loader : function
        A function which takes a single argument, the path to a parameter file,
        and loads the parameters from that file. By default, torch.load,
        which produces state_dict dictionaries.
    ...
    Returns:
    Any
        The state_dict loaded from the first checkpoint for the specified recoverable.
    """
    # Ensure there is at least one checkpoint in the list
    if not checkpoint_list:
        raise ValueError("No checkpoints available to load from.")

    # Get the first checkpoint
    first_checkpoint = checkpoint_list[0]
    print(checkpoint_list)
    # Extract the path for the specified recoverable from the first checkpoint
    recoverable_path = first_checkpoint.paramfiles[recoverable_name]

    # Load and return the state dictionary for the recoverable
    state_dict = parameter_loader(recoverable_path, map_location="cpu")
    return state_dict


def load_best_checkpoint(ckpt, recoverable_name, parameter_loader=torch.load):
    if not ckpt:
        raise ValueError("No checkpoint here")

    recoverable_path = ckpt.paramfiles[recoverable_name]
