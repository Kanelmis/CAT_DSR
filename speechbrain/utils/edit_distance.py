...
def wer_details_by_utterance(
    ...
    for key, ref_tokens in ref_dict.items():
        ...
        details_by_utterance.append(utterance_details)
        print(f"Number of Edits: {utterance_details['num_edits']}")
       # print(f"Insertions: {utterance_details['insertions']}")
       # print(f"Deletions: {utterance_details['deletions']}")
       # print(f"Substitutions: {utterance_details['substitutions']}")
...
def wer_summary(details_by_utterance):
    ...
    wer_details = {
        "WER": WER,
        "SER": 100.0 * num_erroneous_sents / max(1, num_scored_sents),
        "num_edits": num_edits,
...
# Add helper function to calculate CER
def _batch_to_dict_format(ids, seqs):
    return dict(zip(ids, seqs))


def cer_details_for_batch(ids, refs, hyps, equality_comparator):
    """Batch interface for CER calculation."""
    refs_dict = _batch_to_dict_format(ids, refs)
    hyps_dict = _batch_to_dict_format(ids, hyps)
    return cer_details_by_utterance(refs_dict, hyps_dict, equality_comparator)


def cer_details_by_utterance(
    ref_dict,
    hyp_dict,
    compute_alignments=False,
    scoring_mode="strict",
    ...
    """Computes detailed CER information for each single utterance."""
    details_by_utterance = []
    for key, ref_chars in ref_dict.items():
        # Initial structure of details
        utterance_details = {
            "key": key,
            "scored": False,
            "hyp_absent": key not in hyp_dict,
            "hyp_empty": False,
            "num_edits": None,
            ...
            "substitutions": 0,
            "alignment": None,
            "ref_chars": ref_chars if compute_alignments else None,
            "hyp_chars": None,
        }

        if key in hyp_dict:
            hyp_chars = hyp_dict[key]
            utterance_details["hyp_chars"] = hyp_chars if compute_alignments else None
            utterance_details["hyp_empty"] = (len(hyp_chars) == 0)
            table = op_table(ref_chars, hyp_chars, equality_comparator)
            ops = count_ops(table)
            utterance_details.update({
                "num_edits": sum(ops.values()),
                "insertions": ops["insertions"],
                "deletions": ops["deletions"],
                "substitutions": ops["substitutions"],
                "alignment": table if compute_alignments else None,
                "CER": 100.0 * sum(ops.values()) / max(1, len(ref_chars))
            })
        elif scoring_mode == "all":
            utterance_details.update({
                "num_edits": len(ref_chars),
                "CER": 100.0,
                "deletions": len(ref_chars),
                "alignment": []
            })
        elif scoring_mode == "present" and key not in hyp_dict:
            continue  # Skip scoring this utterance
        elif scoring_mode == "strict" and key not in hyp_dict:
            raise KeyError(f"Key {key} in reference but missing in hypothesis and strict mode on.")
        
        details_by_utterance.append(utterance_details)

    return details_by_utterance

def cer_summary(details_by_utterance):
    """Computes summary statistics for CER."""
    total_edits, total_chars = 0, 0
    num_scored_sents, num_absent_sents = 0, 0  # Initialize counters for scored and absent sentences

    for detail in details_by_utterance:
        total_edits += detail["num_edits"]
        total_chars += detail["num_ref_chars"]
        if detail.get("scored", True):  # Assume sentences are scored unless specified otherwise
            num_scored_sents += 1
        if detail.get("hyp_absent", False):  # Check if the hypothesis was absent
            num_absent_sents += 1

    # Calculate overall CER, expressed as a percentage
    overall_cer = (total_edits / total_chars) * 100 if total_chars > 0 else float('inf')

    # Sum up the edits of each type
    insertions = sum(d["insertions"] for d in details_by_utterance)
    deletions = sum(d["deletions"] for d in details_by_utterance)
    substitutions = sum(d["substitutions"] for d in details_by_utterance)

    return {
        "CER": overall_cer,
        "num_edits": total_edits,
        "num_scored_chars": total_chars,
