...
from speechbrain.dataio.cer import print_cer_alignments, print_cer_summary
...
class CerErrorRateStat(MetricStats):
    """A class for tracking error rates (e.g., CER).

    The handling of tokens and characters is controlled by the merge_tokens,
    split_tokens, and the space_token settings.
    """
    def __init__(
        self,
        merge_tokens=False,
    ...
    def append(
        ...
        if ind2lab is not None:
            ...
            target = ind2lab(target)
        if self.merge_tokens:
            predict = merge_char(predict, self.space_token)
            target = merge_char(target, self.space_token)
        if self.split_tokens:
            predict = split_word(predict, self.space_token)
            target = split_word(target, self.space_token)
        if self.extract_concepts_values:
            predict = extract_concepts_values(predict, self.keep_values, self.tag_in, self.tag_out, self.space_token)
            target = extract_concepts_values(target, self.keep_values, self.tag_in, self.tag_out, self.space_token)
        

        scores = cer_details_for_batch(ids, target, predict, self.equality_comparator)
        self.scores.extend(scores)

    def summarize(self, field=None):
        """Summarize the error rate and return relevant statistics."""
        self.summary = cer_summary(self.scores)
        self.summary["character_error_rate"] = self.summary["CER"]
    ...
    def write_stats(self, filestream):
        ...
        if not self.summary:
            self.summarize()
        print_cer_summary(self.summary, filestream)
        print_cer_alignments(self.scores, filestream)
