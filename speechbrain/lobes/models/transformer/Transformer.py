...
class TransformerInterface(nn.Module):
    ...
    def __init__(
        ...
        free_num_encoder_layers=0,
        free_num_decoder_layers=0,
    ):
        super().__init__()
        self.causal = causal
        ...
        if num_encoder_layers > 0:
            ...
            if encoder_module == "transformer":
                ...
                freeze_encoder_layers(self.encoder, free_num_encoder_layers)
                check_if_layers_frozen(self.encoder, free_num_encoder_layers)
                assert (
                    normalize_before
        ...
        if num_decoder_layers > 0:
            ...
            freeze_decoder_layers(self.decoder, free_num_decoder_layers)
            check_if_layers_frozen(self.decoder, free_num_decoder_layers)
...
def freeze_layers(layers, n):
    """ Freezes the first 'n' layers of a given set of layers. """
    for layer in layers[:n]:
        for param in layer.parameters():
            param.requires_grad = False

def freeze_encoder_layers(encoder, n):
    """ Freezes the first 'n' layers of an encoder. """
    freeze_layers(encoder.layers, n)  # assuming all these encoders have a .layers attribute
    print("freezed " + str(n) + " encoder layers")

def freeze_decoder_layers(decoder, n):
    """ Freezes the first 'n' layers of a decoder. """
    freeze_layers(decoder.layers, n)  # assuming the decoder has a .layers attribute
    print("freezed " + str(n) + " decoder layers")


def check_if_layers_frozen(layers, n):
    """ Checks if the first 'n' layers of a given set of layers are frozen. """
    count = 0
    for layer in layers.layers[:n]:
        count = count + 1
        frozen = all(not param.requires_grad for param in layer.parameters())
        if frozen:
            print(f"Layer {count} is correctly frozen.")
        else:
            print(f"Layer {count} is not frozen.")
