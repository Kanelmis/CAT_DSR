...
To run this recipe, do the following:isolated 
...
#start new wandb run to track the script
wandb.init(
        #Set where it will be logged
        project="p4_np1",
        )
...
class ASR(sb.core.Brain):
    ...
    def compute_objectives(self, predictions, batch, stage):
        ...
        if stage != sb.Stage.TRAIN:
            ...
            if current_epoch % valid_search_interval == 0 or (
                ...
                print("using p_seq")
                hyps = [seq.tolist() for seq in p_seq.argmax(-1)]
                predicted_words = []
                for utt_seq in hyps:  # greedy decoding, terminate at eos
                    if hparams["eos_index"] in utt_seq:
                        end_index = utt_seq.index(hparams["eos_index"])
                        utt_seq = utt_seq[:end_index]
                    words = tokenizer.decode_ids(utt_seq)
                    predicted_words.append(words.split(" "))
            ...
            self.acc_metric.append(predicted_words,target_words, p_seq, tokens_eos, tokens_eos_lens)
        return loss        
    ...
    def on_evaluate_start(self, max_key=None, min_key=None):
        ...
        # find the checkpoint based on the lowest wer
        ckpts = self.checkpointer.find_checkpoint(
                max_key=None, min_key="WER"
                )
        print(ckpts)
        # Assuming 'checkpoints' is the list of sorted Checkpoint namedtuples
        model_state_dict = sb.utils.checkpoints.load_best_checkpoint(ckpts,recoverable_name="model")
        self.hparams.model.load_state_dict(model_state_dict, strict=True)
            
        ckpt = self.checkpointer.load_checkpoint(ckpts)
        
        self.hparams.model.eval()
        print("Loaded the checkpoint with highest acc")
    ...
    def on_stage_end(self, stage, stage_loss, epoch):
        ...
        current_epoch = self.hparams.epoch_counter.current
        elapsed_time = time.time() - self.stage_start_time # calculate the elasped time
        stage_stats = {}
        if stage == sb.Stage.TRAIN:
            stage_stats["train_loss"] = stage_loss
            stage_stats["train_time"] = elapsed_time # log training time
            self.train_stats = stage_stats
            wandb.log({"train_loss": stage_stats["train_loss"],
                       "train_time": stage_stats["train_time"]})
        else:
        ...
        if stage == sb.Stage.VALID:
            ...
            # change the name to distinguish between vliad and test stage
            stage_stats["valid_loss"] = stage_loss
            stage_stats["valid_time"] = elapsed_time # log validation time
            epoch_stats = {
                "epoch": epoch,
                "lr": lr,
                "steps": steps,
            ...
            wandb.log({"acc": stage_stats["ACC"], "wer": stage_stats["WER"],"cer": stage_stats["CER"], "valid_loss":stage_stats["valid_loss"], "valid_time": stage_stats["valid_time"]})
            ...
            # Removed since all the avearged process is deleted. 
...
def dataio_prepare(hparams):
    ...
    sb.dataio.dataset.set_output_keys(
        datasets,
        ["id", "sig", "wrd", "tokens_bos", "tokens_eos", "tokens", "wav"],
    )
...
if __name__ == "__main__":
    ...
    for k in test_datasets.keys():  # keys are test_clean, test_other etc
        ...
        print(asr_brain.hparams.test_wer_file)
        asr_brain.evaluate(
            test_datasets[k],
            max_key="ACC",
