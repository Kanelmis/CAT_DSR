...
# Base YAML file name
base_yaml="transfer_ua_torgo_control.yaml"

# Base output folder path prefix (excluding freeze configuration)
base_output_folder="results/transfer_control_ua_torgo_"
...
for i in {1..4}; do
    ...
    sed -i -e "s|freeze_num_decoder_layers: 0|freeze_num_decoder_layers: ${i}|" \
           -e "s|output_folder: !ref results/transfer_control_ua_x/|output_folder: !ref ${base_output_folder}freezeD${i}/|" $new_yaml
...
for i in {1..12}; do
    ...
    sed -i -e "s|freeze_num_encoder_layers: 0|freeze_num_encoder_layers: ${i}|" \
           -e "s|output_folder: !ref results/transfer_control_ua_x/|output_folder: !ref ${base_output_folder}freezeE${i}/|" $new_yaml
    echo "Created and modified $new_yaml"
done
